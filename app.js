
import AudioPlayer from './AudioPlayer.js';

const player = new AudioPlayer('.audioPlayer', [
    { name: 'Dance Monkey', url: 'media/audios/Dance_Monkey.mp3' },
    { name: 'Memories', url: 'media/audios/Memories.mp3' },
    { name: 'Girls Like You', url: 'media/audios/Girls_Like_You.mp3' },
    { name: 'Señorita', url: 'media/audios/Senorita.mp3' },
    { name: 'On My Way', url: 'media/audios/On_My_Way.mp3' },
    { name: 'Música Pop', url: 'media/audios/Musica_para_la_magnana.mp3' },
]);
